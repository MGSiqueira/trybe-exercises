...
function reduceNames() {
  return books.reduce((string, book, i, array) =>  i === array.length - 1 ? string + `${book.author.name}.` : string + `${book.author.name}, `, '');
}
...
function longestNamedBook() {
  return books.reduce((maior, book) => maior.name.length > book.name.length ? maior : book).name
}
